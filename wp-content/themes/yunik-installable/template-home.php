...
get_header(); des_yunik_print_menu(false);
	...
	switch($homeType){
		case "slider":
			if (get_post_meta($this_page_id, 'parallaxEffect_value', true) == 'yes') des_yunik_print_slider($this_page_id, true);
			else des_yunik_print_slider($this_page_id);
		break;
		case "image": case "video":
			?>
			<section id="home" class="homepage_parallax <?php echo $homeType; if (get_post_meta($this_page_id, 'parallaxEffect_value', true) == 'yes') echo " parallax"; ?>">
				<?php
					if ($homeType == "image"){
						$media = get_post_meta($this_page_id, 'homeParallaxMedia_value', true);
						$media = explode("|!|",$media);
						?>
						<div id="parallax-home" <?php if (get_post_meta($this_page_id, 'parallaxEffect_value', true) == 'yes') echo 'class="parallax" data-stellar-ratio="0.5" '; ?> style="background-image: url(<?php echo $media[1]; ?>);background-size:cover;text-align:center;">
							<?php des_yunik_print_intro($this_page_id); ?>
						</div>
						<?php
					} else {
						?>
						<div id="parallax-home" <?php if (get_post_meta($this_page_id, 'parallaxEffect_value', true) == 'yes') echo 'class="parallax" data-stellar-ratio="0.5"'; ?>>
							<?php des_yunik_print_intro($this_page_id, true); ?>
						...
						if (get_post_meta($this_page_id, 'homeVideoSource_value', true) == 'youtube'){
							$controls = (get_post_meta($this_page_id, 'homeVideoControls_value', true) == 'yes') ? "true" : "false";
							?>
							<div class="player" style="display:block; margin: auto; background: rgba(0,0,0,0.5)" data-property="{videoURL:'<?php echo get_post_meta($this_page_id, 'homeYoutubeLink_value', true); ?>',  optimizeDisplay:true, showControls:<?php echo $controls; ?>,containment:'#parallax-home',startAt:0,mute:<?php echo (get_post_meta($this_page_id,'homeVideoMuted_value', true) == 'yes') ? "true" : "false"; ?>,autoPlay:true,player:true,loop:true,opacity:1,stopMovieOnBlur:true}"></div>
							<?php
						}
					}
				?>
	...
	?>
	<section class="page_content section_page-<?php echo $this_page_id; ?>" id="section_page-<?php echo $this_page_id; ?>" data-section-title="<?php echo $thepost->post_title; ?>">
		<div class="container">
		<?php
			if (vc_is_inline()){
				wp_reset_query();
