...
if ( isset($_GET['page']) && $_GET['page'] == DESIGNARE_STYLE_OPTIONS_PAGE ){
	//options actions
	add_action('init', 'designare_init_style_options_functionality');  
	add_action('init', 'designare_set_style_options'); 
}
 ...
 */
function designare_init_style_options_functionality(){
	global $designare_style_options;

	$designare_style_options=array();
	...
	$designare_options_files=array('styles');
...
function designare_set_style_options(){
	...
	if ( $_GET['page'] == DESIGNARE_STYLE_OPTIONS_PAGE ) {
	
		if (isset($_REQUEST['action'])){
			if ( 'save' == $_REQUEST['action'] ) {
				
				//verify the nonce
				if ( empty($_POST) || !wp_verify_nonce($_POST['designare-theme-style-options'],'designare-theme-update-style-options') ){
					print 'Sorry, your nonce did not verify.';
					exit;
				} else {
					
					if (!get_option(DESIGNARE_SHORTNAME.'_style_first_save')){
						update_option(DESIGNARE_SHORTNAME.'_style_first_save','true');
					}
					...
					$doc->save($filename);		
					header("Location: themes.php?page=".DESIGNARE_STYLE_OPTIONS_PAGE."&saved=true");
					die;
				}
			}	
		}
...
function designare_init_options_functionality(){
	...
	//load the files that contain the options
	$designare_options_files=array('general', 'header-menu', 'widgetsarea', 'sliders', 'newsletter', 'translation', 'social', 'customcss', 'data');
	foreach($designare_options_files as $file){
		require_once(DESIGNARE_OPTIONS_PATH.$file.'.php');
	}
}
 ...
 */
function designare_theme_style_options_admin() {
	global $designare_style_options, $designare_style_options_manager;

	$designare_style_options_manager=new DesignareStyleOptionsManager(DESIGNARE_THEMENAME, DESIGNARE_IMAGES_URL, DESIGNARE_UTILS_URL, DESIGNARE_UPLOADS_URL, DESIGNARE_VERSION);
	$designare_style_options_manager->set_options($designare_style_options);

	if ( isset($_REQUEST['saved'] )) {
		$designare_style_options_manager->print_saved_message();
	}
	if ( isset($_REQUEST['reset'] )) {
		$designare_style_options_manager->print_reset_message();
	}

	$designare_style_options_manager->print_heading("");
	$designare_style_options_manager->print_options();
	$designare_style_options_manager->print_footer();
}
 ...
 * Calls the style options manager to print the Style Options page.
 */
function designare_theme_demos_admin() {
	global $designare_demos_manager;

	$designare_demos_manager=new DesignareDemosManager(DESIGNARE_THEMENAME, DESIGNARE_IMAGES_URL, DESIGNARE_UTILS_URL, DESIGNARE_UPLOADS_URL, DESIGNARE_VERSION);

	$designare_demos_manager->print_heading("");
	$designare_demos_manager->print_options();
	$designare_demos_manager->print_footer();
}
...
function designare_add_style_options($option_arr){
	global $designare_style_options;

	foreach($option_arr as $option){
		$designare_style_options[]=$option;
	}
}
...
function des_get_all_google_fonts(){
	...
	try{	
		...
		for ($i = 0; $i < count($fontArray); $i++){
			 array_push($yunik_google_fonts, array('id'=>$fontArray[$i]['css-name'],'name' => $fontArray[$i]['font-name']));
		}	
	} catch(Exception $e) {
		$yunik_google_fonts = array(	array( 'name' => "Cantarell", 'variant' => ':r,b,i,bi'),
