...
function designare_admin_init(){
	...
	if(isset($_GET['page']) && ( $_GET['page']==DESIGNARE_OPTIONS_PAGE || $_GET['page']==DESIGNARE_STYLE_OPTIONS_PAGE || $_GET['page']==DESIGNARE_DEMOS_PAGE)){
		...
		echo "<div class='yunik_logo_type hidden'>".get_option('yunik_logo_type')."</div>";
		echo "<div class='yunik_fixed_menu hidden'>".get_option('yunik_fixed_menu')."</div>";
		echo "<div class='yunik_header_after_scroll hidden'>".get_option('yunik_header_after_scroll')."</div>";
		echo "<div class='yunik_header_shrink_effect hidden'>".get_option('yunik_header_shrink_effect')."</div>";

		if (get_option(DESIGNARE_SHORTNAME."_show_sec_footer") == "on"){
			if (get_option(DESIGNARE_SHORTNAME."_footer_display_logo") == "on"){
				echo "<div class='yunik_footer_logo_type hidden'>".get_option('yunik_footer_logo_type')."</div>";	
			}
			if (get_option(DESIGNARE_SHORTNAME."_footer_display_social_icons") == "on"){
				echo "<div class='yunik_footer_display_social_icons hidden'>".get_option('yunik_footer_display_social_icons')."</div>";	
			}
		}
	}

	if(defined('DESIGNARE_PORTFOLIO_POST_TYPE') && $current_screen->id==DESIGNARE_PORTFOLIO_POST_TYPE){
	...
	if(isset($_GET['page']) && defined('DESIGNARE_PORTFOLIO_POST_TYPE') && $_GET['page']==DESIGNARE_PORTFOLIO_POST_TYPE){
		wp_enqueue_script('jquery');
		wp_enqueue_script('jquery-ui-core');
...
/* if (is_admin() && isset($_GET['activated']) && $pagenow == "themes.php" ) {
    //Do redirect
    header( 'Location: '.admin_url().'admin.php?page='.DESIGNARE_DEMOS_PAGE.'&activated=true' ) ;
} */
		...
		echo '<script>jQuery(document).ready(function($) {
				...
				$(".slider").each(function(){
					...
					$(this).empty().slider({
						...
						max: 100,
						slide: function( event, ui ) {
							if ($(this).hasClass("opacity-slider")){
								$( "#"+$(this).attr("title") ).val( ui.value + "%" );
							} else {
								$( "#"+$(this).attr("title") ).val( ui.value + " px" );	
							}
						}
				...
				designareStyleOptions.init({cookie:true});
		});</script>
		<!--[if IE 9]>
		<style type="text/css">
		.tab_navigation ul li.ui-tabs-selected a.tab span, .tab_navigation ul li.ui-tabs-selected a.tab span{
		...
		if (isset($_GET['dgtt'])){
			...
			<script type="text/javascript">
				jQuery(window).load(function(){
					jQuery('a[href=#tab_navigation-1-<?php echo $_GET['dgtt']; ?>]').click();
				});
			</script>
			<?php	
		}
...
function designare_add_theme_menu(){
	...
	add_theme_page( DESIGNARE_THEMENAME, DESIGNARE_THEMENAME." Style Options", 'delete_pages', DESIGNARE_STYLE_OPTIONS_PAGE, 'designare_theme_style_options_admin', DESIGNARE_LIB_URL.'/images/designare.png');
	add_theme_page( DESIGNARE_THEMENAME, DESIGNARE_THEMENAME." Demos", 'delete_pages', DESIGNARE_DEMOS_PAGE, 'designare_theme_demos_admin', DESIGNARE_LIB_URL.'/images/designare.png');
}
...
<div class="alignleft"><?php if (!function_exists('icl_object_id')) previous_posts_link('<span>&laquo;</span> '.des_text('_previous_text')); else previous_posts_link('<span>&laquo;</span> '.__('Older Entries','yunik')); ?></div>
<div class="alignright"><?php if (!function_exists('icl_object_id')) next_posts_link(des_text('_next_text').' <span>&raquo;</span>'); else next_posts_link(__('Newer Entries','yunik').' <span>&raquo;</span>'); ?></div>
</div>
	<?php
	}
}
