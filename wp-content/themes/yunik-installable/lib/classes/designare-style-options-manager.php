 ...
 */
class DesignareStyleOptionsManager{
	...
	/**
	 * The main constructor for the DesignareStyleOptionsManager class
	 ...
	 */
	function DesignareStyleOptionsManager($themename, $images_url, $utils_url, $uploads_url, $version){
	...
	function print_heading($heading_text){
		...
		echo '<div id="designare-content-container"><form method="post" id="designare-style-options">';
		if ( function_exists('wp_nonce_field') ){
			wp_nonce_field('designare-theme-update-style-options','designare-theme-style-options');
		}
		echo '<div id="sidebar"><div id="logo"></div><div id="navigation"><ul style="visibility:hidden;">';
		...
		echo '</ul></div></div><div id="content"><div id="header"><h3 id="theme_name">'.$this->themename.' <span>v.'.$this->designare_version.'</span></h3><a class="online-doc" href="http://designarethemes.net/docs/yunik-wp/" target="_blank"><i class="icon-file-alt"></i> Online Documentation</a><a class="support" target="_blank" href="http://support.designarethemes.net/forums/forum/central/yunik-ultimate-multi-concept-wordpress-theme/"><i class="icon-comments-alt"></i> Help & Support Forum</a></div><input type="submit" value="Save Changes" class="save-button" /><div id="options_container">';
	}
	...
	function print_subnavigation($value, $i){
		...
		if($value['subtitles']){
			echo '<div id="tab_navigation-'.$i.'" class="tab_navigation" style="visibility:hidden;"><ul>';
			foreach($value['subtitles'] as $subtitle){
				if (!isset($subtitle['id'])) $subtitle['id'] = "";
	...
	function print_textarea($value){
		...
		$input_value = $this->get_field_value($value['id'], $std);
		echo ' <textarea name="'.$value['id'].'" id="'.$value['id'].'" class="option-textarea" cols="" rows="">'.$input_value.'</textarea>';
		$this->close_option($value);
	}
	...
	function print_upload($value){
		...
		echo '<script type="text/javascript">jQuery(document).ready(function($){
			designareStyleOptions.loadUploader(jQuery("div#'.$value['id'].'_button"), "'.$this->designare_utils_url.'upload-handler.php", "'.$this->designare_uploads_url.'");
		});</script>'; 
	...
	function print_opacity_slider($value){
		echo $this->before_option_title.$value['name'].$this->after_option_title;
		$input_value = $this->get_field_value($value['id'], $value['std']);
		echo '<div class="slider opacity-slider" title="'.$value['id'].'"></div><input class="option-input slider-input" name="'.$value['id'].'" id="'.$value['id'].'" type="text" value="'.$input_value.'" style="text-align: center; border: 0; background: none; color: #314572; width: 201px; padding: 5px 0 0 15px; margin: 0; font-style: italic;" />';
		$this->close_option($value);
	}
	...
	function print_upload_from_media($value){
		...
		<div class="uploader" id="upload-<?php echo $value['id']; ?>">
		  <?php echo '<input class="option-input upload" name="'.$value['id'].'" id="'.$value['id'].'" type="text" value="'.$input_value.'" style="display:none;"/>'; ?>
		  <textarea type="textarea" style="display:none;"><?php echo $input_value; ?></textarea>
		  <input class="button buttonUploader" name="<?php echo $value['id']; ?>_button" id="<?php echo $value['id']; ?>_button" value="Select Media" style="width:auto;text-align:center;"/>
		  <img class="previewimg" src="<?php echo $input_value; ?>" style="position: relative; float: left; display: inline-block; clear: left; left: 220px; margin-top: 10px; max-width:300px;"/>
		</div>
		
		<script type="text/javascript">
			jQuery(document).ready(function($){
				
			  var uploadbox = '<?php echo "#upload-".$value["id"]; ?>';
	...
	function print_stylebox($value, $type){
		...
		foreach ($value['options'] as $option) {
			...
			echo '<li onclick="jQuery(this).parents(\'.sub-navigation-container\').find(\'#'.DESIGNARE_SHORTNAME.'_style_color\').attr(\'value\',\''.$option.'\'); jQuery(this).parents(\'.sub-navigation-container\').find(\'.color-preview\').css(\'background-color\',\'#'.$option.'\');" style="'.$style.'" class="'.$class.'"><a class="style-box" title="'.$option.'" href=""></a></li>';
		} 
		echo '</ul></div>';
		$this->close_option($value);
	}
	...
	function print_custom($value){
		...
		foreach($value['fields'] as $field){
			...
			switch($field['type']){
				...
				case 'upload':
					...
					echo '<script type="text/javascript">jQuery(document).ready(function($){
								designareStyleOptions.loadUploader(jQuery("div#'.$field['id'].'_button"), "'.$this->designare_utils_url.'upload-handler.php", "'.$this->designare_uploads_url.'");
						});</script>';
					$preview=$field['id'];
					$is_textarea[]="false";
				break;
		...
		//if ($value['id'] != 'yunik_reset_options_button')
		...
		$urlassign = get_template_directory_uri()."/lib/functions/getstyleXML.php";
		
		echo '<script type="text/javascript">
			jQuery(document).ready(function($){
				designareStyleOptions.setCustomFieldsFunc("'.$value['id'].'", ["'.$idsString.'"], ["'.$namesString.'"], ['.$textareaString.'] , "'.$prv.'",  "");
				jQuery(\'#yunik_export_style_options_button\').css({\'position\':\'relative\',\'float\':\'left\'}).attr(\'target\',\'_blank\').unbind().click(function(){
