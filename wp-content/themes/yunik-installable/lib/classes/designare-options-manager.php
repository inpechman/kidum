...
class DesignareOptionsManager{
	...
	function print_heading($heading_text){
		...
		foreach ($this->options as $value) {
			...
			if($value['type']=='title'){
				...
				echo '<li><span><a href="#navigation-'.$i.'"><div class="'.strtolower(str_replace(" ", "_", $value['name'])).'"></div><i class="fa fa-'.strtolower($namestr).'-painel" style="position:relative;float:left;line-height:47px;margin-left:20px;margin-right:5px;font-size: 15px;"></i><span>'.$value['name'].'</span></a></span></li>';
				$i++;
			}
		}

		echo '</ul></div></div><div id="content"><div id="header"><h3 id="theme_name">'.$this->themename.' <span>v.'.$this->designare_version.'</span></h3><a class="online-doc" href="http://designarethemes.net/docs/yunik-wp/" target="_blank"><i class="fa fa-file-text-o"> </i>&nbsp; Online Documentation</a><a class="support" target="_blank" href="http://support.designarethemes.net/forums/forum/central/yunik-ultimate-multi-concept-wordpress-theme/"><i class="fa fa-comments-o"></i> Help & Support Forum</a></div><input type="submit" value="Save Changes" class="save-button" /><div id="options_container">';
	}
	...
	function print_fakeinput($value){
		...
		echo "<textarea style='display:none;' id='".$value['id']."' name='".$value['id']."' value='".$input_value."' class='option-textarea' cols='' rows=''>";
		echo $input_value;
		echo "</textarea>";
	}
	...
	function print_textarea_wysiwyg($value){
		...
		$input_value = $this->get_field_value($value['id'], $std);
		echo ' <div class="textarea_wysiwyg_container"><textarea style="display:none;" name="'.$value['id'].'" id="'.$value['id'].'" class="option-textarea" cols="" rows="">'.$input_value.'</textarea>';
		...
		<script type="text/javascript">
			jQuery(document).ready(function(){
				jQuery('#wp-<?php echo $value['id']."_editor"; ?>-wrap').siblings('p.submit').children('#submit').click(function(e){ 
					e.stopPropagation();e.preventDefault();
					jQuery('#<?php echo $value['id']; ?>').html( jQuery('iframe#<?php echo $value['id']."_editor"; ?>_ifr').contents().find('#tinymce.<?php echo $value['id']."_editor"; ?>' ).html() ) });
			});
		</script>
		<?php
		$this->close_option($value);
	...
	function print_website_loaders($value){
		...
		echo '<input  name="'.$value['id'].'" id="'.$value['id'].'" type="hidden" value="'.$input_value.'" /><div class="loaders-container">';
		...
		foreach ($value['options'] as $option) {
			...
			if ($class == "load2" || $class == "load3" || $class == "load6"){
				echo '<div onclick="jQuery(this).closest(\'.loaders-styles-holder\').find(\'#'.DESIGNARE_SHORTNAME.'_website_loader\').attr(\'value\',\''.$option['id'].'\'); jQuery(this).addClass(\'selected\').siblings().removeClass(\'selected\');" class="loader-item '.$selected.'"><div class="loaders-style-box '.$class.'"><div class="loader"></div></div></div>';
			} else {
				echo '<div onclick="jQuery(this).closest(\'.loaders-styles-holder\').find(\'#'.DESIGNARE_SHORTNAME.'_website_loader\').attr(\'value\',\''.$option['id'].'\'); jQuery(this).addClass(\'selected\').siblings().removeClass(\'selected\');" class="loader-item '.$selected.'"><div class="loaders-style-box loader-inner '.$class.'">'.$divs.'</div></div>';
			}
	...
	function print_custom($value){
		...
		echo '<script type="text/javascript">
			jQuery(document).ready(function($){
				...
				jQuery(\'#yunik_export_options_button\').css({\'position\':\'relative\',\'float\':\'left\'}).attr(\'target\',\'_blank\').unbind().click(function(){
	...
	function print_goto($value){
		...
		?>
		<div class="des_go_to button"><a target="_blank" href="<?php echo $pageURL."?page=designare_style_options&dgtt=".$value['name']; ?>"><?php echo $value['desc'] ?></a></div>
		<?php
	}
