...
class ContactInfo_Widget extends WP_Widget {
	function ContactInfo_Widget() {
		$widget_ops = array('classname' => 'contact_info_widget', 'description' => __('Minimalist contact information.', 'yunik'));
		parent::__construct(false, 'DESIGNARE _ Contact Info', $widget_ops);
	}
...
?>  
       ...
       <p><label for="<?php echo $this->get_field_id('type'); ?>">&#8212; <?php _e('Info Type', 'yunik'); ?> &#8212;<br/>
       <select id="<?php echo $this->get_field_id('type'); ?>" name="<?php echo $this->get_field_name('type'); ?>" onchange="if (jQuery(this).val() === 'other') jQuery('#<?php echo $this->get_field_id('customicon'); ?>').parent().add(jQuery('#<?php echo $this->get_field_id('customicon'); ?>').parent().siblings('.icons-container')).add(jQuery(this).closest('p').siblings('.selectedIcon')).css('display','block'); else jQuery('#<?php echo $this->get_field_id('customicon'); ?>').parent().add(jQuery('#<?php echo $this->get_field_id('customicon'); ?>').parent().siblings('.icons-container')).add(jQuery(this).closest('p').siblings('.selectedIcon')).css('display','none');">
       ...
       <input style="display:none;" class="widefat" id="<?php echo $this->get_field_id('customicon'); ?>" name="<?php echo $this->get_field_name('customicon'); ?>" type="text" value="<?php echo $customicon; ?>" />
       <label for="<?php echo $this->get_field_id('customicon'); ?>">&#8212; <?php _e('Custom Icon', 'yunik'); ?> &#8212;<br/>
	       <?php
       ...
       <p><label for="<?php echo $this->get_field_name('content'); ?>">&#8212; <?php _e('Info Content', 'yunik'); ?> &#8212;      
       <textarea class="widefat" rows="5" cols="20" id="<?php echo $this->get_field_id('content'); ?>" name="<?php echo $this->get_field_name('content'); ?>" value="<?php echo $content; ?>"><?php echo $content; ?></textarea>
       </label></p>
       
       <label for="<?php echo $this->get_field_name('center'); ?>" style="position:relative;float:left;width:100%;margin: 0 0 10px 0px;"><input type="checkbox" name="<?php echo $this->get_field_name('center'); ?>" id="<?php echo $this->get_field_id('center'); ?>" value="<?php echo $center; ?>" <?php if ($center) echo "checked"; ?> onchange="jQuery(this).val(jQuery(this).is(':checked'));">&nbsp;Centered ?</label>
