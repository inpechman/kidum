(function ($) {
	...
	window.VcVerticalTabsView = vc.shortcode_view.extend({
				...
				setTimeout(function(){
					switch(el.$style){
						case 'icon':
							...
							break;
						case 'text':
							jQuery('.wpb_el_type_des_fa').css('display','none');
							break;
					}
				}, 1000);
			});
	    ...
	    cloneModel:function (model, parent_id, save_order) {
	        ...
	        model_clone = Shortcodes.create({shortcode:model.get('shortcode'), id:vc_guid(), parent_id:parent_id, order:new_order, cloned:(model.get('shortcode') === 'verticaltab' ? false : true), cloned_from:model.toJSON(), params:new_params});
