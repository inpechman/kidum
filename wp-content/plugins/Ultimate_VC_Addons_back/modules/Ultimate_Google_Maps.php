		...
		function display_ultimate_map($atts,$content = null){			
			...
			{			
				...
				$output .= "var marker_$id = new google.maps.Marker({
                    ...
                    if ($grayscale === "enable"){
                        $output .= "google.maps.event.addListenerOnce(map_$id, 'tilesloaded', function(){
								    jQuery('#$id img').addClass('designare_grayscale');
								});
								jQuery('#$id').hover(function(){ jQuery(this).find('img').removeClass('designare_grayscale'); }, function(){ jQuery(this).find('img').addClass('designare_grayscale'); });
								";
